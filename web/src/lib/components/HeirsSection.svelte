...
<section class="mb-6">
	<h2 class="text-lg font-semibold mb-3">{$t('form.heirs')}</h2>
	{#each groups as group}
		{@const heirs = HEIRS.filter((h) => h.group === group)}
		<div class="mb-4">
			<h3 class="text-xs uppercase tracking-wide text-gray-400 mb-2">{groupLabel[group]}</h3>
				...
				{#each heirs as heir}
					...
					>
						<div class="flex-1 min-w-0">
							<span class="text-sm font-medium">{$t(heir.labelKey)}</span>
						</div>
						<div class="flex items-center gap-2 ms-3">
							<button
								...
								aria-label="decrease"
							>-</button>
							<span class="w-5 text-center text-sm tabular-nums">{count}</span>
							<button
								type="button"
